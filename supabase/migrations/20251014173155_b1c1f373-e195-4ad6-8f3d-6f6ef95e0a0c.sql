-- Create RPC function to get users with their correct roles from user_roles table
-- This ensures admin_client users are correctly differentiated from admin users
...
BEGIN
  ...
  SELECT public.has_role(p_requesting_user_id, 'admin'::app_role) INTO v_is_admin;
  
  -- Get requesting user's client_id
  SELECT id_client INTO v_user_client_id
  ...
  IF v_is_admin THEN
    ...
    WHERE 
      (p_filter_client_id IS NULL OR p.id_client = p_filter_client_id)
      AND (p_filter_role IS NULL OR ur.role::text = p_filter_role)
    ORDER BY p.email;
    RETURN;
  END IF;
